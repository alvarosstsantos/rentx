...
let CreateCarSpecificationUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  constructor(carsRepository, specficicationsRepository) {
  ...
  async execute({
    ...
    if (!carExists) {
      throw new _AppErrors.AppError("car does not exists");
    }

    const specficications = await this.specficicationsRepository.findByIds(specifications_id);
