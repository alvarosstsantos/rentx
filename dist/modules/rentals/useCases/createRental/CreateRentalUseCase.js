...
let CreateRentalUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (carUnavailable) {
      throw new _AppErrors.AppError("car is unavailable for rent");
    }

    const rentalOpenToUser = await this.rentalsRepository.findOpenRentalByUser(user_id);

    if (rentalOpenToUser) {
      throw new _AppErrors.AppError("there is already a rental open for this user");
    }
    ...
    if (compare < minRentHours) {
      throw new _AppErrors.AppError("invalid return date");
    }
